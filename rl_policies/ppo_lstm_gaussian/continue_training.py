from ...pushing_envs.pushing_env_lstm_gaussian import PushingEnv
...
def get_last_model():
    files = glob.glob(f'checkpoints/ppo_lstm_gaussian_*/*')
    models = []
    for file in files:
        folders = file.split("/")
...
model_name = f"ppo_lstm_gaussian_part_{last_model_part+1}_{last_model_checkpoint}/"
...
model = RecurrentPPO.load(last_model_file, tensorboard_log=f"./tensorboard/")
